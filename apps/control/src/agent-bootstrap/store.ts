...
type Forward = Readonly<{ v: 1; kind: 'binding_forward'; agentParticipantId: ParticipantId; bindingId: BindingId }>;
...
export function createAgentBindingStore(deps: Readonly<{
  ...
  async function readScoped(address: BindingAddress): Promise<BindingLookup> {
    ...
    if (!await ensureIndex(address, record.binding.bindingId)) return { kind: 'unavailable' };
    return { kind: 'found', record };
  }

  async function followForward(ownerId: OwnerId, roomId: RoomId, forward: Forward): Promise<BindingLookup> {
    const address = { ownerId, roomId, agentParticipantId: forward.agentParticipantId };
    const target = await readScoped(address);
    if (target.kind !== 'found') return { kind: 'unavailable' };
    return target.record.binding.bindingId === forward.bindingId ? target : { kind: 'unavailable' };
  }
  ...
  async function migrate(address: BindingAddress): Promise<BindingLookup> {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      ...
      const marker: Forward = {
        v: 1, kind: 'binding_forward', agentParticipantId: address.agentParticipantId, bindingId: legacy.binding.bindingId,
      };
  ...
  async function findParticipant(address: BindingAddress): Promise<BindingLookup> {
    ...
    if (legacy.kind === 'record') {
      ...
      if (forward) {
        const target = await followForward(address.ownerId, address.roomId, forward);
        if (target.kind !== 'found') return { kind: 'unavailable' };
        return forward.agentParticipantId === address.agentParticipantId ? target : readScoped(address);
      }
  ...
  async function claimSession(input: BindingAddress & Readonly<{ harness: string; sessionId: string }>): Promise<SessionClaim> {
    ...
    const existing = await findParticipant(input);
  ...
  async function putParticipant(
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      ...
      // Without the roll-forward gate, a legacy record stays authoritative. It
      // may be reused but must not be replaced by an ignored scoped write.
      if (scoped.kind === 'absent' && current.kind === 'found') return { kind: 'unavailable' };
  ...
  async function findBinding(bindingId: BindingId | string): Promise<BindingLookup> {
    ...
    if ('v' in decoded) address = decoded;
    else {
    ...
    const found = await findParticipant(address);
    if (found.kind === 'absent') return { kind: 'unavailable' };
    return found.kind === 'found' && found.record.binding.bindingId !== bindingId ? { kind: 'absent' } : found;
  }
  ...
  async function updateBinding(
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      const found = await findBinding(bindingId);
      ...
      const address = {
        ownerId: found.record.binding.ownerId, roomId: await roomForBinding(bindingId),
        agentParticipantId: found.record.binding.agentParticipantId,
      };
      if (address.roomId === null) return 'unavailable';
      const legacyKey = agentBindingStoreKeys.legacy(address.ownerId, address.roomId);
  ...
  async function roomForBinding(bindingId: BindingId | string): Promise<RoomId | null> {
    const read = await readJson(agentBindingStoreKeys.index(bindingId));
    if (read.kind !== 'record') return null;
    return decodeIndex(read.record.value)?.roomId ?? null;
  }
...
function decodeForward(value: unknown): Forward | null {
  const object = exactObject(value, ['v', 'kind', 'agentParticipantId', 'bindingId']);
  if (!object || object.v !== 1 || object.kind !== 'binding_forward'
    || !identifier(object.agentParticipantId) || !identifier(object.bindingId)) return null;
