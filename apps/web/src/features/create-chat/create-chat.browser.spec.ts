...
// Browser-verified against the production CreateChatScreen with synthetic,
...
test('CreateChatScreen completes an unnamed chat with two intros and keeps submit/copy reachable at narrow viewports', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    const titleField = page.getByLabel('Chat name (optional)');
    ...
    const submit = page.getByRole('button', { name: 'Create chat' });
    await submit.waitFor();
    await page.waitForFunction(() => {
      const button = [...document.querySelectorAll('button')].find(node => node.textContent === 'Create chat');
      return button instanceof HTMLButtonElement && !button.disabled;
    });
    await submit.click();

    // Busy phases announce progress and the form is no longer editable.
    await page.getByText('Creating the chat…').waitFor();
