...
describe('createChannelController', () => {
  it('subscribes before reading and does not let a late initial read replace a live snapshot', async () => {
    ...
    const port: ChannelUiPort = {
      ...
      installCommand: async () => 'khala connect channel-link',
    };
    const controller = createChannelController(port, { roomId, generation: 2 });
  ...
  it('ignores another binding generation and loads onboarding only for disconnected agents', async () => {
    let listener: ((value: AgentPresenceSnapshot) => void) | null = null;
    const installCommand = vi.fn(async () => 'khala connect https://khala.example/channel/1');
    const port: ChannelUiPort = {
    ...
    const controller = createChannelController(port, { roomId, generation: 4 });
