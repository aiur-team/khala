...
import entry, { type OpenCodePluginClient, parseOpenCodeEnvelope } from '@aiur/khala/opencode';
...
// The installed OpenCode plugin entry, `@aiur/khala/opencode`'s default export exactly as
// OpenCode loads it, against the real internal launcher. The session discovers, requests
...
async function loadPlugin(client: OpenCodePluginClient, version: string) {
  ...
  try {
    const hooks = await entry.server({ client, directory: DIRECTORY });
...
function batchOf(result: string) {
  ...
  const parsed = parseOpenCodeEnvelope(envelope!);
...
async function ownerView(w: World) {
  ...
  const { bindingId } = bindings.json.bindings[0].binding as { bindingId: string };
...
describe('the installed OpenCode plugin against the internal launcher', () => {
  it('delivers to the idle bound session, reads with khala_read and acknowledges on the next Khala call', async () => {
    ...
    const delivered = parseOpenCodeEnvelope(prompts[0]!.text)!;
