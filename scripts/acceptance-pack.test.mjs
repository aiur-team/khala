...
function fakeGate(calls, effect = () => {}) {
  return ({ packageDirectory, outputDirectory }) => {
...
test('packs a clean tree and records its commit and sha256 beside the tarball', t => {
  ...
  const calls = [];
  const result = packForAcceptance({ repositoryRoot: directory, outputDirectory: path.join(directory, 'out'), gate: fakeGate(calls), install: noInstall });
...
test('refuses a tarball whose packing changed the tree, and records no provenance', t => {
  const { directory } = repository(t);
  const calls = [];
  const gate = fakeGate(calls, packageDirectory => fs.writeFileSync(path.join(packageDirectory, 'package.json'), '{"changed":true}\n'));
  const result = packForAcceptance({ repositoryRoot: directory, outputDirectory: path.join(directory, 'out'), gate, install: noInstall });
  assert.match(result.errors.join('\n'), /changed the tree/);
  assert.equal(fs.existsSync(path.join(directory, `out/aiur-khala-0.1.0.tgz${PROVENANCE_SUFFIX}`)), false);
});

test('packs from a fresh worktree at the commit, so a stale ignored web bundle never reaches the gate', t => {
  const { directory, commit } = repository(t);
  const stale = 'apps/web/dist/internal-web/index.html';
  fs.mkdirSync(path.dirname(path.join(directory, stale)), { recursive: true });
  fs.writeFileSync(path.join(directory, stale), 'bundle from another commit');
  const installed = [];
  const seen = [];
  const gate = fakeGate([], packageDirectory => {
    const tree = path.resolve(packageDirectory, '../..');
    const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: tree, encoding: 'utf8' }).trim();
    seen.push({ tree, head, stale: fs.existsSync(path.join(tree, stale)) });
  });
  const result = packForAcceptance({ repositoryRoot: directory, outputDirectory: path.join(directory, 'out'), gate, install: worktree => installed.push(worktree) });
  assert.deepEqual(result.errors, []);
  assert.equal(seen.length, 1);
  assert.notEqual(seen[0].tree, directory);
  assert.deepEqual(seen[0], { tree: seen[0].tree, head: commit, stale: false });
  assert.deepEqual(installed, [seen[0].tree]);
  assert.equal(fs.existsSync(seen[0].tree), false, 'the worktree is removed after packing');
  assert.equal(execFileSync('git', ['worktree', 'list', '--porcelain'], { cwd: directory, encoding: 'utf8' }).match(/^worktree /gm).length, 1);
});
