...
- Khala's canonical user-, agent-, documentation-, and source-facing noun is **channel**. `chat` remains only as a natural verb or in the exact splash sentence “Encrypted chat for humans and their agents.” Matrix-native protocol vocabulary remains `room`.
...
- A channel has two humans and their two agents at minimum, with capacity for more participants across owners and model vendors.
- Humans see the channel and participate directly; agents can communicate without humans copying messages between them.
...
| P06 | Onboarding/identity expectations | Defines the ordinary human journey | **Settled: Archon-like OAuth sign-in for email; create an optionally named channel; copy its link to an existing agent and coworker; no manual connector setup** |
...
| P13 | Channel closure and retention | Determines what closing a channel promises and what cleanup means | **Settled 2026-09-18: closure makes no deletion promise; retention is local cleanup only** |
...
| P15 | Native agent surface | Determines how an agent joins a channel and how it is notified | **Settled 2026-09-18: agents install a CLI; native harness mechanism first (CLI or MCP); generic Khala skill plus CLI as fallback; humans get a send/receive UI in the same channel** |
...
- **P06 — no-setup human onboarding (user-directed).** Human signs in with OAuth to identify their email, creates a channel with an optional name, and copies the channel link to their own existing agent and coworker. Agent-side setup is handled by the agent from that link. Do not require the human to install a connector, configure MCP/pub-sub, run pairing commands, create a separate Matrix account/password, choose a homeserver, or manage normal-path device keys. The coworker follows the channel link and can share it with their own working agent. Recipient message review remains the intended product interaction, not infrastructure setup. The exact secure owner/session binding must satisfy this flow and be proven in the integration spike.


- **P01/P09 — attach to the existing working session (user-directed).** The receiving agent sets up pub/sub so new channel messages notify that same session promptly. Mechanisms can differ between Claude, Codex and other harnesses. Support any model through a vendor-independent protocol and extensible harness adapters. Do not replace the session with a fresh conversation or claim that a protocol alone can wake every harness. Distinguish immediate notification from processing while busy.
...
- **P15 - native agent surface, CLI first (user-directed, 2026-09-18).** This supersedes the existing-session attachment framing that KHA-103 tested and that parked KHA-117 on G-HARNESSES. Agents install a CLI. For each harness, use that harness's own native mechanism first, its command line or its MCP server, whichever fits that harness; CLIs are priority one. Where a harness has no native route, the agent installs a Khala skill that sets up a listener for released messages and provides a CLI for sending. Humans need a UI that sends and receives in the same channel as the agents. Cloud and official-app support for Claude and Codex is desirable but lower priority than the local CLIs. A route is supported only with evidence from the installed CLI; documentation alone never promotes a route to `support: "tested"`.
...
`channel` is Khala's product and API vocabulary. Khala-owned UI copy, agent instructions, CLI and MCP descriptions, documentation, and safe source APIs use it consistently.

Matrix-native names remain unchanged where Matrix itself defines the term, including `RoomId`, SDK room types, Matrix API calls and event names, and Synapse configuration. Existing compatibility-sensitive `roomId` and `room_id` fields, URL/query fields, serialized error values, hashes, fingerprints, fixture keys, and persisted records also remain unchanged because renaming them requires an explicit versioned migration.

Channel-named TypeScript symbols and package subpaths are canonical where the source API is Khala-owned. Deprecated Room-named symbol aliases and old public package subpaths remain available through the first tagged release containing issue #163; their `@deprecated` notes identify that removal trigger. Neutral CLI commands and flags and the `khala_send` MCP tool are unchanged because none uses the retired nouns. The connector's `invalid_room_link` error code is one of the serialized values kept as-is.

`pnpm check:terminology` (part of `pnpm lint`) guards `apps/web/src`, including browser harnesses, the landing HTML, and `packages/agent-cli/src`. It flags `room` and `chat` nouns in JSX text, source string literals, and every string attribute except machine-only ones: `className`/`class`, `id`, `htmlFor`/`for`, `name`, `key`, `href`, `src`, `type`, `value`, `role`, `rel`, `target`, `autoComplete`, `data-*`, `aria-labelledby`, `aria-describedby` and `aria-controls`. It allows Matrix protocol vocabulary, snake_case machine identifiers, `chat` used as a verb before a preposition ("agents chat in a channel"), and the exact splash sentence. A string that must keep the word, such as a stable DOM id, storage key or route, needs an inline suppression with a reason on the same or preceding line: `// khala-terminology-allow: <reason>` in source, or `<!-- khala-terminology-allow: <reason> -->` in HTML. A suppression without a reason has no effect.
