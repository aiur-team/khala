...
| `committing` | Source revision is frozen and final destination state is reconciled | Failure unfreezes the internal room unless the external link already committed. |
...
The human can cancel before `committing`; the created external room is then reported as an orphan requiring explicit cleanup rather than hidden deletion. Messages may continue internally during preparation. Carry-history repeatedly copies bounded deltas until one final delta remains. Finalization then takes a short write freeze, copies and verifies that delta, commits the authoritative link, and activates the paused destination bindings. No agent can publish or receive in the destination before that activation barrier.
...
| Prepare | Show room creation, history copy/catch-up, and each agent's paused destination status. Reload resumes from the journal. |
...
| Commit | Announce the brief internal write freeze; copy the final delta, link rooms, activate agents, then navigate to the external room. Failure unfreezes and remains retryable. |
...
| Complexity | `complexity:4` |
| Scope | Durable conversion states; immutable mode/room/roster snapshot; authenticated hosted room creation; create-result reconciliation; per-agent verification and batch human approval; paused destination bindings; re-invite status/retry/skip; short final write freeze; atomic link-and-activation barrier; cancel/orphan reporting. |
...
| Complexity | `complexity:4` |
| Scope | Snapshot the internal SQLite log; encode bounded archive chunks; authorize every manifest/chunk write against the signed-in conversion owner, journaled operation, and destination room; copy bounded deltas until one final delta remains; send through the external encrypted-room composition; persist acknowledgements; reconcile and resume; expose bounded read-only context pages to re-invited agents. |
...
| Acceptance | Human can complete both choices; blocked agents keep conversion incomplete until joined/skipped; denial is terminal and mints no grant; post-create cancel shows orphan cleanup; reload resumes; all actions are keyboard-operable with managed focus and named controls; expiry/progress/errors/completion are announced; imported bodies render as inert text/code only. |
| Tests | Playwright both choices, sign-in failure/return, pre/post-create cancel, denial, expiry, retry, partial-agent failure, reload, keyboard/focus/live announcements; use script/image/link/fake-approval markup and assert no execution, external loads, active links, or body-derived controls; fake provider loses responses and still yields one room/import. **Wrong implementation caught:** imported history emitted as live events wakes fake agents or receipts. |
