...
| A2 | Conversion creates one `channel-access-journal` request per selected agent. The human may decide those exact requests in one `channel-access-inbox` batch UI, but each grant remains individual; any changed, stale, revoked, or unsupported session remains unapproved and visible. |
...
| `agents_pending` | One `channel-access-journal` request per selected, verified agent plus its `pending/granted/ready/skipped/blocked` outcome | The human may grant the displayed requests in one `channel-access-inbox` batch decision, but every grant remains individual. `channel-access-activation` readiness feeds a conversion-paused destination binding; no selected agent is silently bound or allowed to publish/receive. |
...
| `activating` | The internal channel is read-only, the authoritative link is committed, and per-binding release of the conversion pause is journaled | Resume release forward for bindings whose individual access flow is ready; never infer readiness from the batch decision or reopen the internal channel after authority has moved. |
...
The human can cancel before `committing`; the created external channel is then reported as an orphan requiring explicit cleanup rather than hidden deletion. Messages may continue internally during preparation. Carry-history attempts at most three catch-up rounds. If one maximum-sized delta remains, finalization takes a short write freeze; otherwise the UI enters `drain_required` and asks the human to pause internal agents and writes for a bounded drain. Failure before link commit resumes the internal channel. Link commit atomically marks it read-only and records activation intent; any later failure resumes forward through `activating`. The launcher and conversion service never bind an agent directly: each selected agent gets its own journal request and individual grant. `channel-access-activation` must report the access flow ready before commit, but the resulting destination binding remains conversion-paused and cannot publish or receive until the authoritative link commits and `activating` releases it.
...
| Confirm | Choose history mode, review the exact agent roster, and approve the immutable conversion summary before channel creation. This freezes the plan; it does not create a grant or bind an agent. |
| Prepare | Create the destination channel, then create one access request per selected agent. Show channel creation, up to three history catch-up rounds, and each agent's paused destination status. If the backlog stays large, offer a bounded final drain that pauses internal agents and writes. Reload resumes from the journal. |
| Resolve agents | Review the individual access requests in the shared inbox. One batch action may grant the displayed set as a UI convenience, but it grants only those exact requests individually. Retry a blocked agent or explicitly skip it; unresolved or not-yet-ready agents disable final commit. Denying a pairing claim is terminal, mints no grant, and tells the claimant to request a new code. |
...
| One batch decision over individual access requests | Makes “same agents” practical while preserving one auditable grant and activation result per selected agent. | Changed generations or unavailable sessions need explicit per-agent resolution; the batch UI cannot silently bind or widen the displayed set. |
...
| Blocked by | `setup-cli-plan`; `internal-launcher`; `pairing-code-control-protocol`. |
...
| Blocked by | None. |
| E09 conflict risk | High with `room-discovery` (#144), `setup-cli` (#143), and harness integrations (#139/#140/#142), which all consume binding identity. Land this contract early: it is a prerequisite for `channel-access-activation` and every same-owner external channel with two or more agents. |
...
| Complexity | `complexity:4` |
| Scope | Durable conversion states including pre-link and post-link recovery; immutable mode/channel/roster snapshot; authenticated hosted channel creation; create-result reconciliation; explicit visibility choice; per-agent verification; one `channel-access-journal` request for every selected agent after destination creation; a human decision over those exact requests through `channel-access-inbox`; per-request readiness from `channel-access-activation` into conversion-paused destination bindings; re-invite status/retry/skip; final write pause; atomic link/read-only transaction; resumable activation barrier and per-binding pause release; cancel/orphan reporting. |
| Out of scope | History payload import, imported-history rendering, automatic internal deletion, launcher-owned agent binding, silently replacing failed agents, or treating a batch UI action as one shared grant. |
...
| Acceptance | Start-fresh transfers zero messages; the human explicitly chooses channel visibility and an omitted choice defaults to `secret`; repeated operation creates one external channel; exact session/generation identities are re-verified; every selected agent produces one individual journal request only after destination creation; the human grants only those exact requests through the shared inbox, even when using one batch action; the launcher never silently binds agents; conversion waits for per-request readiness from `channel-access-activation` while each destination binding remains conversion-paused; blocked agents keep conversion incomplete; commit requires every agent ready or explicitly skipped; link commit makes the internal channel read-only, invalidates old local listing references, and records activation intent before any paused binding is released; post-link recovery only resumes release forward. |
| Tests | Lose the create response and reconcile the same destination; assert no access request exists before destination creation; omit visibility and require `secret`, then select each supported visibility explicitly; retain an old local listing reference and require it to become invalid after commit; mutate the conversion choice and require conflict; select three agents and require three journal requests and three individual grants from one batch decision; inject an unselected request into the batch and require it to remain ungranted; attempt a launcher-side direct bind and require refusal; withhold one access-readiness result and require commit to remain disabled; use stale/revoked/unsupported sessions and require blocked statuses; prove a ready destination binding remains unable to publish/receive before link commit and pause release; fail immediately before link commit and require the source to unfreeze; fail after link commit but before releasing every binding and require the source to stay read-only while release resumes; assert start-fresh sends no source message. **Wrong implementation caught:** a launcher that turns the roster into bindings bypasses the individual journal, inbox grant, conversion pause, and activation records. |
| Blocked by | `local-sqlite-channel-store`; `internal-launcher`; `external-channel-composition`; `pairing-code-control-protocol`; `pairing-code-connector`; `multi-agent-bindings`; `channel-access-journal`; `channel-access-inbox`; `channel-access-activation`; `channel-discovery-contract`; `internal-channel-discovery`. |
...
| Blocked by | `local-sqlite-channel-store`; `start-fresh-conversion`. |
...
| Blocked by | `local-sqlite-channel-store`; `external-channel-composition`; `start-fresh-conversion`; `imported-history-contract`. |
...
| Scope | Hosted pairing claim display, approval, denial, expiry, and terminal-state feedback; show the verified harness/session identity, generation, and target channel; reuse the shared admission prompt component owned by `channel-access-inbox`. |
...
| Files/packages | `apps/web/src/features/pairing/**` and browser tests; the shared prompt component from `channel-access-inbox`; no `apps/internal/` changes. |
...
| Blocked by | `pairing-code-control-protocol`; `channel-access-inbox`. |
| E09 conflict risk | Highest with `room-discovery` (#144), which owns the shared access-inbox prompt. Reuse that component and do not fork its admission semantics. |
...
| Acceptance | Human can complete both history choices and explicitly choose visibility, with `secret` as the default; blocked agents keep conversion incomplete until ready/skipped; non-convergent history offers a clear paused-drain choice; post-create cancel shows orphan cleanup; reload resumes; old local listing references stop navigating after commit; a post-link failure shows forward activation recovery and never offers source resume; all actions are keyboard-operable with managed focus and named controls; progress/errors/completion are announced; imported bodies render as inert text/code only. |
...
| Blocked by | `local-web-entry`; `internal-launcher`; `external-channel-composition`; `pairing-code-connector`; `multi-agent-bindings`; `start-fresh-conversion`; `imported-history-contract`; `history-transfer`; `pairing-approval-ui`. |
...
Land `pairing-code-control-protocol` and `multi-agent-bindings` first, independently of `internal-core`; `multi-agent-bindings` must precede `channel-access-activation` and any same-owner multi-agent external channel. The shared path is `mcp-inbox-batch` → `mcp-result-piggyback` → `listening-mode-pull` → `setup-cli-plan`. Land the local, discovery, and channel-access dependencies in their own declared order through `internal-launcher`, `channel-access-inbox`, and `channel-access-activation`; then promote `pairing-code-connector`, keeping its CLI/MCP implementation in new modules with minimal registration diffs. Promote `pairing-approval-ui` after `channel-access-inbox`. Once every blocker named above is available, land `start-fresh-conversion` → `imported-history-contract` → `history-transfer` → `make-external-journey`. Every local-conversion PR must adopt the final `apps/internal/` and external-channel composition boundaries.
