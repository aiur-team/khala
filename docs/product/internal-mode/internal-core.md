...
| Endpoint | `POST /api/v1/channels/:channelId/stop` with `{v: 1, targets: null}` for every active binding, or `targets: [{bindingId, generation}]` for exact recorded targets. A stale or unknown target is refused with 409. The reply is `{v: 1, outcome: 'stopped' \| 'partial', stopped, remaining}`, and `partial` names each remaining binding with `revoke_failed` or `descriptor_pending`. |
