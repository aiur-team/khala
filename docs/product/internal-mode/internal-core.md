...
| Runtime descriptor and bindings | Enforce one active internal launcher per OS user with a root-level lock; port incrementing handles unrelated listeners, not a second Khala launcher. Keep the human bootstrap credential in `<channel>/launch.json`; atomically write the stable root-level 0600 `active.json` descriptor containing `{v, channelId, origin, transportCapability}` for installed local clients. Before grant, the descriptor permits transport discovery only. `/khala join <channel-url>` delegates the request and human grant to `channel-access-journal` and `channel-access-inbox`; after grant, atomically write that transport descriptor plus `{grantRef, bindingId, bindingCapability}` into the agent's own 0600 `discovery/<principal>/grant.json`, so two agent sessions of one OS user each hold their own grant; no grant is ever copied into `active.json`: the installed Codex/OpenCode `mcp-serve` entry and `codex-hook` act only as the calling session's own `grant.json`, found from the session the call names, and one-shot CLI calls and long-lived MCP/plugin clients can reopen that exact file for participant authority. Setup stores only `--internal-descriptor <stable-active-path>`, never a port or token. Participant capabilities are launch-scoped, stored server-side only as keyed hashes, compared in constant time, and all invalidated on shutdown/resume. A resumed launcher writes a transport-only `active.json`; the agent's next `join` (or Claude session status) reseeds its own descriptor from it, resumes its connected activation by operation ID without a grant, the server validates that the durable binding is still active before issuing fresh authority, and the agent atomically writes it into the descriptor. `join` never reports `connected` while the descriptor holds no grant. Stop revokes the server generation and removes the binding fields from every granted descriptor that names it. Never accept attribution from a body or caller-selected binding ID. Modes isolate other OS users, not processes sharing the operator's uid. |
