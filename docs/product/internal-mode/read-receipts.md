...
|---|---|---|---|
| Codex native CLI `0.154.0` — hooks (main route) + local inbox; `codex-idle-wake` proof-gated | Hooks are the main interactive route (decision 31); `codex-idle-wake` stays `unproven` until it passes its own proof (decision 34). `codex queue` proves only `harness_queued`; the queue ID is not correlated to the consumed user item. Released bytes are read from the durable CLI inbox. | **Queued at agent session** until stronger evidence arrives. | Possible through shared `khala read` and the next authenticated Khala call. The current listener cursor is not evidence: it advances immediately after stdout accepts a line. |
...
| conflict risk | **High** with `local-sqlite-channel-store` on the owner-local read model; medium with `read-receipt-ui` and the internal acceptance tickets, which consume but do not own this projection. |
...
| blocked-by | `read-receipt-recording`; `opencode-session-bridge`; `opencode-delivery-contract`; `listening-mode-contract`; `listening-mode-pull`; `mcp-inbox-batch`; `mcp-result-piggyback`; `setup-cli-opencode`; `opencode-listening-routes`; `channel-access-journal`; `channel-access-inbox`. |
...
| blocked-by | `read-receipt-contract`; `read-receipt-recording`; `read-receipt-observation`; `read-receipt-ui`; `codex-read-receipts`; `claude-read-receipts`; `opencode-read-receipts`; `internal-protocol-acceptance`; `internal-browser-acceptance`; `authenticated-loopback-server`; `local-web-entry`; `local-sqlite-channel-store`. |
| conflict risk | **High** with `internal-protocol-acceptance` and `internal-browser-acceptance` in fake CI/Playwright composition; medium with `local-sqlite-channel-store` on read-only fact inspection. It consumes existing live-run outputs and must not edit their orchestration. |

## Recommended order

`read-receipt-contract → read-receipt-recording → read-receipt-observation → read-receipt-ui → (codex-read-receipts ∥ claude-read-receipts ∥ opencode-read-receipts) → read-receipt-acceptance`. Recording remains dormant until a harness ticket returns a token, so the consumer/read-model/UI chain lands before production v2 facts appear. The harness tickets wait for the exact interactive-session, setup, access, and delivery contracts named above. Receipt acceptance reuses `internal-protocol-acceptance`, `internal-browser-acceptance`, and existing live-run logs/channel-store facts; it creates no second live run or harness.
