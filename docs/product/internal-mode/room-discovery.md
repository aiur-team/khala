...
| Assumption | Internal owner decisions require a short-lived user-presence proof from the trusted launcher/OS that is unavailable to the agent's 0600 launch-token file. If `internal-core` cannot supply that distinction, internal discovery cannot satisfy D11. |
...
It is deliberately separate from post-admission `AdapterCapability`; neither capability contains `approve`, `admit`, `set_policy`, or membership mutation. Internal mode derives the equivalent short-lived discovery scope from the launch token plus verified native session. That shared reachability token never authorizes a decision: approval also requires a single-request, browser-session-bound user-presence proof minted by the trusted launcher/OS confirmation and never written to the agent's 0600 file.
...
|---|---|---|
| `RoomListing` | `v`, opaque `listingRef`, bounded untrusted `title`, `visibility`, `serviceKind`, `requestState` | No `roomId`, owner, roster, counts, topic, timestamps, activity, invite, content, or history |
...
| `AccessRequestStatus` | `pending_owner`, `grant_ready`, `connecting`, `connected`, `repair_required`, `denied`, `expired`, `revoked`, `unavailable` | Contains no grant; `repair_required` is a known post-approval connector failure, while privacy-sensitive unknown/stale/ineligible/mismatched records collapse to `unavailable` |
...
The agent-facing handlers follow the existing strict projection and `no-store` pattern. `--origin` and every transport redirect reuse bootstrap's exact configured-origin allowlist: HTTPS except loopback, no embedded credentials, and no cross-origin redirect. The human mutation follows the gateway's Origin checks and authenticated principal context. In internal mode, Host/Origin plus the shared launch token authenticate only the local session; approve/deny additionally consumes the request-bound browser user-presence proof above. An agent holding the valid file token and a verified session cannot mint or replay that proof.
...
4. Approval is an authenticated owner command bound to request revision, room, and verified session generation. Internal approval also consumes its one-time browser user-presence proof. A stale or duplicate decision reconciles; it never creates a second membership.
...
| Tests | Round-trip all three visibility values and every finite status; reject extra `roomId`, `participants`, `lastActivity`, content, grant, or `admit` fields; reject oversized titles/pages/cursors and normalize control characters; add a compile-time fixture proving `RoomDiscoveryPort` exposes no `admit` member; reject expired/wrong-origin/wrong-generation credential and grant-exchange projections. **Wrong-implementation test:** a response containing `roomId` or `participantCount` must fail strict decoding. |
...
|---|---|
| `complexity` | `complexity:4` |
| Scope | Extend the connector/bootstrap composition so each submitted request journals its operation ID locally, resumes after restart, and uses bounded-backoff status polling (or an equivalent authenticated wake signal) until terminal. On `grant_ready`, reserve a local device, consume the grant through RD1's connector-only proof-bound exchange, redeem with device ID and proof, activate the returned binding/capability, initialize review trust, and acknowledge readiness. |
...
| Acceptance criteria | Grant is requester/origin/session-generation/proof-key/device bound, single-use, short-lived, and never returned by status/CLI/MCP or logged/persisted in plaintext; only the connector exchange can consume it; polling is bounded/jittered and the durable operation resumes after restart; `connected` is returned only after local activation ack; deterministic activation failure is `repair_required` and resumes the same device/binding after repair; admission ambiguity maps to `unavailable` until reconciled. |
| Tests | Status/CLI/MCP grant absence; grant theft/cross-session exchange; wrong device/proof/origin/generation; replay; expiry; approval while connector is offline; restart from pending/grant-ready; bounded polling; response loss or crash between grant consumption and admission; crash after admission and before activation; repair-required recovery; duplicate readiness ack; review baseline. **Wrong-implementation test:** an owner-approved request with no connector activation acknowledgment must never report `connected`. |
...
|---|---|
| `complexity` | `complexity:3` |
| Scope | Supply the internal catalog/visibility adapter from the local room store, bind discovery to the launch token plus verified native session, persist requests, compose the same owner prompt semantics in the local UI, and obtain a short-lived request-bound user-presence proof from the trusted launcher/OS for each decision. Default local rooms to `private`, where eligibility is an explicit stable-principal allowlist with no same-owner fallback; `secret` hides them and `public` means any verified agent on that local service. |
| Out of scope | SQLite room/event schema design, general launcher/server security beyond consuming `internal-core`'s user-presence port, external conversion, harness delivery, duplicating RD4 UI. |
...
| Acceptance criteria | Restart preserves visibility, explicit per-agent eligibility, and pending decisions; secret is absent; two same-owner local agents receive different private listings according to their allowlists; rebind revalidates generation; launch token, verified session, and forged Host/Origin still cannot approve without fresh user presence; approval produces one ready binding in review. |
| Tests | SQLite/restart integration; allowlist add/remove and rebind across two verified same-owner agents; token-only attacker; valid token plus verified session with absent/replayed presence proof; Host/Origin rejection; visibility mutation; pending request expiry; UI composition smoke. **Wrong-implementation test:** an agent holding the valid file token must be unable to approve or mint a grant without launcher/OS user presence. |
