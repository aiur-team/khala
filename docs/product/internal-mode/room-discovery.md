...
| Prompt spam or operation collision | Fingerprint the first resolved target under requester/session/origin/operation; mismatched reuse returns `unavailable`; hard-cap pending work at five per agent and 50 per owner, enforce a five-minute requester/channel creation cooldown (lifted only once the owner approved the earlier request and then ended it, as Stop does; a request revoked before approval, such as on a revision change, still cools down) and durable owner mutes, and batch owner notifications above 10 new requests/minute | Public channels may still create notification load, especially from distributed accounts |
