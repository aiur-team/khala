...
| `AdmissionGrantExchange` | `operationId`, connector proof, reserved device ID, connector X25519 encryption public key | Connector-only secret-delivery boundary; the first matching exchange binds the encryption-key thumbprint to the authenticated Ed25519 proof-key thumbprint and operation/device, then consumes the one-time grant into a versioned RFC 9180 HPKE Base-mode envelope using DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, and ChaCha20Poly1305. The envelope carries `enc` and ciphertext; HPKE `info` is the fixed label `khala/channel-access-envelope/v1`, and canonical AEAD associated data binds version, operation, requester, origin, session generation, device, and both key thumbprints. Retries return the byte-identical stored envelope without repeating admission or sealing; CLI/MCP/status never receive either secret. |

Grant sealing uses a standard construction rather than a bespoke KEM/KDF/AEAD composition. Existing bootstrap redemption is retained for initial online bootstrap, but is insufficient here because an admission may commit while the connector loses the response; exposing the grant through status would break the secret boundary. RFC 9180 supplies a versioned, test-vector-backed sealed response for durable recovery. A compatible audited Node 22 library is **unproven** on this branch: `channel-discovery-contract` must prove one against official vectors before freezing the wire schema, and must stop for a security decision rather than implementing HPKE primitives locally if none is suitable.
...
| RFC 9180 HPKE for recoverable grant results | Standard suite, official vectors, and no status-visible secret after response loss | Adds a vetted dependency; compatible Node 22 library support remains unproven until the contract ticket |
...
| Prompt spam or operation collision | Fingerprint the first resolved target under requester/session/origin/operation; mismatched reuse returns `unavailable`; hard-cap pending work at five per agent and 50 per owner, enforce a five-minute requester/channel creation cooldown and durable owner mutes, and batch owner notifications above 10 new requests/minute | Public channels may still create notification load, especially from distributed accounts |
...
| Tests | Round-trip all three visibility values and every finite status; reject extra `roomId`, `participants`, `lastActivity`, content, grant, `create`, or `admit` fields; reject oversized titles/pages/cursors and normalize control characters; prove private/secret URL requests from an unknown agent can produce only a pending owner request while the same agent remains ineligible for enumeration, and make invalid/ineligible URL outcomes indistinguishable; add a compile-time fixture proving `ChannelDiscoveryPort` exposes no `create`/`admit` member and an adapter fixture requiring human-workflow context plus an idempotency key; prove the selected audited HPKE library on Node 22 with official RFC 9180 vectors before freezing the envelope schema; reject malformed/cross-origin channel URLs, expired/wrong-origin/wrong-generation credential and grant-exchange projections; reject encryption-key reuse with a different proof-key thumbprint or operation/device tuple. **Wrong-implementation test:** a response containing Matrix `roomId` or `participantCount` must fail strict decoding. |
...
| Conflict risk | Medium with `local-sqlite-channel-store` on shared contract names; external persistence remains isolated. High with `external-channel-composition` on hosted route composition. |
...
| Conflict risk | High with hosted runtime/alert composition in `external-channel-composition` and `make-external-journey`; keep the kill-switch and telemetry registration isolated from catalog policy. |
...
| `blocked-by` | `channel-access-journal`, `external-channel-composition`. |
...
| Conflict risk | High with `external-channel-composition` and `start-fresh-conversion` on provider/admission composition. Keep the route and state machine under a dedicated exchange module. |
...
| Acceptance criteria | Private key material is owner-only and never logged; polling is bounded/jittered and the durable tuple resumes after restart; envelope version, associated data, and both thumbprints are verified before activation; a lost response re-fetches the same stored envelope; `connected` appears only after local activation acknowledgement; deterministic activation failure or lost recovery key is `repair_required` and never remints membership; the server envelope is deleted on readiness acknowledgement or its hard expiry; Khala does not launch or terminate the user's agent process. |
| Tests | Approval while connector is offline; restart from pending/approved; bounded polling; X25519 rotation before exchange and rejection after consumption; lost/mismatched private key; envelope tamper/context mismatch; response loss and identical recovery; crash after admission before activation; recovery-envelope deletion; repair-required recovery; duplicate readiness acknowledgement; review baseline. **Wrong-implementation test:** an owner-approved request with no connector activation acknowledgment must never report `connected`. |
...
| `blocked-by` | `channel-discovery-contract`, `channel-access-journal`, `channel-access-activation`, `channel-agent-listing`. |
...
| `blocked-by` | `channel-discovery-contract`, `external-channel-discovery-bootstrap`, `channel-access-journal`, `channel-access-inbox`, `channel-access-grant-exchange`, `channel-access-activation`, `internal-channel-discovery`, `multi-agent-bindings`, `external-channel-composition`, `local-sqlite-channel-store`, `authenticated-loopback-server`, `local-web-entry`. |
...
| `blocked-by` | `channel-create-workflow`, `channel-access-cli-mcp`, `internal-channel-discovery`. |
