...
|---|---|---|---|---|
| `steer` | `PreToolUse` blocks the next tool long enough to inject the batch; `PostToolUse` injects when a message arrived during the tool; `Stop` is the no-more-tools fallback. | **Proven**: a batch arrived during a 20-second Bash sleep at `22:18:00.455Z` and was injected 1 ms after `PostToolUse` at `22:18:02.443Z`. A second run proved `PreToolUse`. | **Proven**: `PreToolUse` injected at `22:22:09.244Z`; the live agent acknowledged at `22:22:14.141Z` and retried the tool. | Native hooks, next tool boundary. Hard abort remains off. |
| `sync` (default) | Ignore `PostToolUse`; pull at `Stop` after the active turn, or at the next `UserPromptSubmit` for an idle notification. | **Proven**: `PostToolUse` stayed silent; `Stop` delivered at `22:19:11.914Z` and the live agent acknowledged. | **Proven**: the next `UserPromptSubmit` delivered at `22:22:47.739Z` and the live agent acknowledged. | Native `Stop` plus `UserPromptSubmit`; optional fixed-content `codex queue` wake. |
