...
| Delivery contracts | `ApprovalCommand`, `PolicySetCommand`, `ReleasedJob` (`releaseFromApproval`/`verifyReleasedJob`), `DeliveryReceipt`, `HarnessCapabilities` v3 (retained v2 decode), `HarnessPort` | `packages/contracts/src/delivery/*.ts` | Approval → release → harness |
