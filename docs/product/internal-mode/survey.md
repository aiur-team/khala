...
5. **Nothing today turns a model's output into a room message automatically.** In the product, agents reply by running `khala send` or calling the `khala_send` MCP tool (`packages/agent-cli`). Internal mode must choose one of two approaches: have agents call `khala_send` against a local server, or have the harness capture each turn's final assistant message and post it.
...
|---|---|---|---|
| Contract port, UI-facing | `RoomPort` (`create`, `prepareIntro`, `resumeIntro`, `send`, `timeline`, `observe`) | `packages/contracts/src/messaging/rooms.ts` | What the web features consume |
...
| Room service | `createRoomService({ principal, actor, device, substrate, journal, limits, clock })` → `RoomPort` | `packages/messaging/src/rooms/index.ts` | Idempotent create, `clientTxnId` send dedupe, intro batches, timeline projection keyed by `eventId`, generation fencing, sha256 digest over `encodeMessageContent` |
| **Substrate SPI (human/room side)** | `RoomSubstrate` (`createRoom`, `findCreatedRoom`, `room`, `sendEvent`, `timeline`, `subscribe`) | `packages/messaging/src/rooms/substrate.ts` | **The pluggable transport.** Finite results (`done`/`rejected`/`unavailable`/`unknown`), no SDK errors |
| Room journal | `RoomJournal` (CAS on revision); `createMemoryRoomJournal` (non-durable) | `packages/messaging/src/rooms/journal.ts` | Device-local send/create intents |
...
```
human browser ──RoomPort──> createRoomService ──RoomSubstrate──> [transport]
                                                                     │
...
- **Browser side:** only on the **unmerged KHA-132 branch**, in `apps/web/src/composition/human/matrix-browser.ts`. There, `class MatrixSubstrate implements RoomSubstrate` feeds `createRoomService`, and `createMatrixBrowserPorts` returns `{ device, room, participant }`. The server side lives in `apps/control/src/composition/human/matrix.ts` (Synapse shared-secret registration, P17 server-minted session).
...
1. **`RoomSubstrate` (local).** An append-only event log per room. It must:
...
3. **A browser transport for (1).** The browser cannot share a Node process, so it needs either:
   - (a) an HTTP + SSE/WebSocket `RoomSubstrate` client in the browser that talks to the local server, with `createRoomService` running in the browser (the same shape KHA-132 uses for Matrix); or
   - (b) a `RoomPort` over HTTP, with `createRoomService` running server-side.

   Option (a) reuses more, because the browser keeps the same `RoomService` semantics and `RoomJournal` it has in production.
...
Storage engine: `node:sqlite` is already the connector's engine, with no new dependency. It is the natural choice for both the room log and the connector ledger. An alternative is append-only JSONL, following the agent-cli inbox's pattern.
...
- A reply path for each (see Headline 5):
  ...
  - **R2, turn capture.** The local harness driver reads the turn's final assistant message: Codex `item/completed` or `turn/completed` notifications (already tracked by `createCodexReceiptTracker`), and Claude `result` stream-json lines. It posts that message as the agent participant. This is deterministic chat, but it is new code, and it bypasses `khala send`.
- Turn-taking and loop limits (see §4 and the questions).
- Prompt framing: the release envelope is JSON. Each model needs a system or intro message that explains the envelope and the other participant. The room service's `prepareIntro` can carry a scripted intro.

---

## 3. Web chat UX (`apps/web`)

| Feature | Port(s) | Main status | Local needs |
|---|---|---|---|
| `features/timeline` (`TimelineScreen`, `createTimelineController`) | `RoomPort` (`timeline` + `observe` + `send`), a viewer `ParticipantView` | Built; inert text rendering; send retry by `clientTxnId` | Local `RoomPort` via `createRoomService` + local substrate. **Core of internal mode.** |
| `features/room` (`RoomScreen`, `AgentPresencePanel`) | `RoomUiPort` (`agents`, `subscribeAgents`, `installCommand`) | Built; HTTP composition in `apps/web/src/composition/human/agent-presence.ts` (polling fetch) | Local presence endpoint: two agents, connection state and route label. `installCommand` returns the local launch hint or can be hidden. |
| `features/create-chat` | `IdentityPort`, `DevicePort`, `RoomPort`, `AdmissionPort`, `ContentLimits` | Built; includes share-link generation | Reuse for "new chat". The share step is hidden or disabled locally. |
...
`netlify.toml` sets a CSP with `connect-src 'self'`, which works if the local server serves both the bundle and the API from one origin. `create-chat`, `join` and `routes.ts` assume `https` share links. Locally the share UI is hidden. Web Crypto `subtle.digest` needs a secure context, and `http://127.0.0.1` and `http://localhost` count as potentially trustworthy, so `digestMessageContent` works. Serving on a LAN IP would break it and return `crypto_unavailable`.
...
| Automatic release (`evaluateAutomaticRelease`) | `packages/policy/src/trust/automatic.ts` | **Needed for hands-free chat.** It releases only events from the one named peer, only after arming, and bounded by `maxCausalDepth` and the caller's budget. |
...
|---|---|---|
| `FakeSubstrate implements RoomSubstrate` | `packages/messaging/src/rooms/fixtures/fakes.ts` | A template for the local substrate: in-memory maps, `landed[]`, listeners. It is under `fixtures/`, which `check:boundaries` bans from production imports and the export map blocks, so it would be **re-implemented** as `local/` production code with durability. It cannot be imported. |
| `createMemoryRoomJournal` | `packages/messaging/src/rooms/journal.ts` | Exported and non-fixture, but not durable. A browser session uses it, or a localStorage/IndexedDB journal. KHA-132 branch has `room-journal.ts`. |
...
| Browser harnesses + `fake-room-port.ts`, `fake-review-port.ts` | `apps/web/src/features/*/browser-harness/` | Show that each feature runs standalone with injected ports. They are dev pages, not an app. |
...
| Owner-only file modes (0700 dir, 0600 files), `O_NOFOLLOW`, EXCLUSIVE SQLite lock | The chat log and ledger hold plaintext. Other local users must not read them. |
...
browser (apps/web        │  HTTP/SSE  ─┐                                                               │
 features + local        │             ▼                                                               │
 main.tsx; RoomService   │   LocalRoomLog (node:sqlite, ~/.local/share/khala/internal/<chat>/room.db)  │
 in browser over         │     ├─ RoomSubstrate impl  (server side of the browser's HTTP substrate)    │
...
- **Reuses:** `createRoomService`, `RoomPort` and all web features; `startSubscription`; connector storage and the dispatcher; policy release and trust; `createCodexHarness`; `khala` CLI and MCP (if R1); the e2e harness and conformance suites.
- **New code:**
  - `LocalRoomLog` (implementing `RoomSubstrate` and `SubscriptionSource`);
...
A single orchestrator process owns a SQLite or JSONL chat log and drives strict alternation:
...
The browser gets a local `RoomPort` implemented directly on that log. The service can still use `createRoomService` over a trivial `RoomSubstrate` so the timeline UI is unchanged. It **skips** the connector subscription, pending store, approval/release and dispatcher.

- **Reuses:** contracts (message shapes, digests, `RoomPort`), the web timeline, room and shell features, and possibly `createRoomService`. It reuses low-level Codex/Claude process knowledge from the evidence, but not `HarnessPort`.
- **Pros:**
...
6. **KHA-132 dependency.** The UX the operator wants to mirror is not on main and is not deployed as a working chat. Building a local `main.tsx` now risks duplicating or conflicting with KHA-132's composition.
...
- Whether the hosted khala.aiur.team will ever render review and controls in the same room page. KHA-132's `renderHumanRoom` stubs them. "The same UX" may mean the timeline only.
- Browser `RoomJournal` durability locally: is memory acceptable, or should it use IndexedDB like KHA-132's `room-journal.ts`?
