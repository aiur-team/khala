...
Implement I2 as a package-local postprocessor for every valid Khala MCP `tools/call` result. It consumes the one durable batch API, appends its oldest-first tokenized batch after the tool's normal content, and leaves acknowledgement to the agent's next Khala call. The later `listening-mode-pull` ticket wires `khala_read` through the same postprocessor and batch format; neither ticket introduces another read, batch, lease, or cursor API.
...
| E09 has one pull operation and one tokenized inbox batch API. MCP uses `khala_read`; the agent's next Khala call acknowledges the prior batch on Khala's side. `HarnessCapabilities.acknowledgement` is owned by `listening-mode-contract` and records `batch_token_next_call` only after evidence passes. | [Shared E09 decisions 1–9](https://github.com/aiur-team/khala/pull/153#issuecomment-5822366740); [decisions 25–30](https://github.com/aiur-team/khala/pull/153#issuecomment-5823138750) | fixed operator decision |
| The `agent-cli` hotspot lands `mcp-inbox-batch` → `mcp-result-piggyback` → `listening-mode-pull`; each ticket adds modules and keeps shared registration diffs minimal. | [Shared E09 decision 18](https://github.com/aiur-team/khala/pull/154#issuecomment-5822379469) | fixed operator decision |
| Product evidence targets the user's own, user-started interactive CLI session. A Khala-hosted app-server route is secondary and cannot satisfy acceptance. | [Operator session decision](https://github.com/aiur-team/khala/pull/154#issuecomment-5822967187); [shared decision 30](https://github.com/aiur-team/khala/pull/153#issuecomment-5823138750) | fixed operator decision |
...
| A local nested `codex exec --ephemeral` nonce attempt failed while initializing its in-process app-server with `Read-only file system`; even a passing hosted attempt would not prove delivery into the user's own interactive Codex CLI. | Local command on 2026-09-24; operator session decision above | **unproven** |
...
| Pull boundary | Expose one postprocessor seam over the shared batch API. `listening-mode-pull` later routes `khala_read` through it; when `khala_read` is the primary tool, compose its already-selected batch once and never read or append it twice. |
...
| Reuse `khala_read` | Keeps one pull operation and one batch format across CLI, MCP, Claude, and OpenCode. | Requires a stable postprocessor seam so `mcp-result-piggyback` can land before `listening-mode-pull`. |
...
| A released peer message prompt-injects a tool-capable host. | Keep the untrusted-content framing. Setup may report an optional hardening check, but delivery into the user's normal interactive session never depends on a restricted profile. |
...
- Replacing native Claude hooks or OpenCode plugin push, or treating a hosted Codex app-server as the primary product/evidence route.
...
| Scope | Wire held binding/generation and the shared batch API into `mcp-serve`; add and strip the shared optional `ackBatchToken` control argument; serialize calls; put the result postprocessor in a new MCP module; append the exact tokenized batch to every valid Khala tool result; expose the single-composition seam that later lets `listening-mode-pull` reuse an already-selected `khala_read` batch; acknowledge only an exact echoed token. |
...
| Complexity | `complexity:2` |
| Scope | Run two real Codex 0.154.0 MCP proofs inside the user's own user-started interactive Codex CLI with unique queued messages: under the declared async-mode setup, prove the agent itself selects `khala_read`, consumes the shared batch, and echoes its token on a later Khala call; separately prove a valid non-read Khala tool result receives a batch through the result postprocessor and the next call acknowledges it. Distinguish agent-issued calls from fixture-issued protocol calls, require an explicit `khala_send` response, and record exact commands, inventory, outputs, and negative claims for the capability owner. |
| Out of scope | Hosted app-server evidence as acceptance, capability-registry edits, promoting untested Codex versions, proving Claude/OpenCode, idle wake, arbitrary-tool injection, restricted-profile delivery gates, or product setup UX. |
...
| Blocked-by | `mcp-result-piggyback`; `listening-mode-pull`; `listening-mode-contract`. |
| Conflict risk | Medium with setup and capability-matrix research. This ticket owns only evidence; `listening-mode-contract` consumes it, owns the acknowledgement enum and advertised support, and may report optional hardening without gating delivery. |

## Recommended sequence

`listening-mode-contract` plus `mcp-piggyback-format-proof` → `mcp-inbox-batch` → `mcp-result-piggyback` → `listening-mode-pull` → `mcp-piggyback-evidence` → capability advertising.

No MCP listening mode is supported by this design alone. Passing end-to-end evidence in the user's own CLI is the only input that may promote MCP-only `async` and `acknowledgement: batch_token_next_call` for a named harness/version; this route does not establish `sync` or `steer`. `listening-mode-contract` owns those capability changes. Setup may report optional hardening, but delivery never depends on a restricted profile.
