---
title: "KHA-122 Create chat and introductions - Plan"
type: feat
date: 2026-09-16
topic: create-chat-composer
...
# KHA-122 Create chat and introductions - Plan

## Goal Capsule

A human can create an optionally named chat, prepare several introduction messages and copy its link.
...
- R1. An authenticated human creates a named or unnamed chat without configuring infrastructure.
- R2. Introduction messages preserve authored order and remain distinct messages suitable for later recipient review.
- R3. Retries do not duplicate the chat or already accepted introduction messages.
...
- F1. Enter an optional title and two introduction messages, create, wait for confirmed availability, then copy the chat link.

### Acceptance examples

- AE1. Covers F1 / R1–R4. The first introduction succeeds and the second times out; retry resumes the unresolved operation without creating a second room or resending the first.
...
- KTD1. `CreateChatScreen` consumes `IdentityPort`, `DevicePort`, `RoomPort` and `AdmissionPort` from `@khala/contracts/messaging` via injected `CreateChatPorts`. It never receives OAuth refresh tokens or raw Matrix credentials. KHA105 is authoritative for all transport shapes.
...
- KTD3. Copy only the canonical server-issued chat URL; the UI cannot construct a Matrix URL or append an owner proof. Use one share affordance for own agent and coworker. Clipboard failure offers a selectable field and never claims success.
...
Owned exports: `CreateChatScreen`, `createChatController`, `CreateChatPorts`, `CreateChatView`. Files are `CreateChatScreen.tsx`, `controller.ts`, `model.ts`, `ports.ts`, `create-chat.css` with adjacent tests. The ports type selects existing operations rather than re-declaring their payloads.

```ts
type IntroDraft = { localId: string; body: string };
type CreateChatPhase = "editing" | "creating" | "preparing_intro" | "resolving" | "sharing" | "ready" | "failed";
type CreateChatView = { phase: CreateChatPhase; title: string; intros: readonly IntroDraft[]; roomId: string | null; shareUrl: string | null; errorCode: string | null };
```
...
stateDiagram-v2
  ...
  creating --> preparing_intro: room accepted
...
If no introductions are entered, proceed directly from confirmed room creation to share issuance. A failure after room creation retains its room ID and operation references; the button says retry the failed phase, never create again. Leave/new-room action warns only when unsent local content would be lost; it does not claim remote cancellation. A refreshed page can resolve known operation IDs only if the upstream contract supports a safe non-secret resume reference; otherwise show unresolved creation and never automatically recreate it. Navigate to an existing room only when its confirmed roomId is still available. A lost in-memory roomId cannot be recovered by inference; offer the available room list or an explicit later new-chat action while explaining the prior outcome is unknown.
...
**Goal:** Keep typed drafts separate from confirmed room/intro state.

**Requirements:** R1–R3; F1; KTD1/KTD2. **Dependencies:** KHA101/105/107.

**Files:** `apps/web/src/features/create-chat/controller.ts`, `apps/web/src/features/create-chat/model.ts`, `apps/web/src/features/create-chat/ports.ts`, `apps/web/src/features/create-chat/controller.test.ts`.

**Approach:** Own local title/message ordering and a single in-flight operation. Use canonical RoomPort create/prepareIntro/resumeIntro and AdmissionPort share boundary. Tag results with controller generation; disposed or wrong-account responses cannot update state.
...
**Verification:** State tests demonstrate no duplicate room/accepted intro across retries.
...
**Files:** `apps/web/src/features/create-chat/CreateChatScreen.tsx`, `apps/web/src/features/create-chat/create-chat.css`, `apps/web/src/features/create-chat/CreateChatScreen.test.tsx`.
...
**Files:** `apps/web/src/features/create-chat/share-link.ts`, `apps/web/src/features/create-chat/share-link.test.ts`, `apps/web/src/features/create-chat/CreateChatScreen.test.tsx`.

**Approach:** Obtain canonical URL from admission; display ready only after success. Copy callback can be injected for tests and host. Keep accepted room identity when issuance times out; inspect/resolve same operation.
...
**Files:** `apps/web/src/features/create-chat/fixtures.test.ts`, `apps/web/src/features/create-chat/create-chat.browser.test.ts`, `apps/web/src/features/create-chat/README.md`.
...
1. Two pending intros, partial acceptance, unknown outcome and empty chat render distinct states.
...
`pnpm --filter @khala/web typecheck`; `pnpm --filter @khala/web test -- src/features/create-chat`; `pnpm --filter @khala/web test:browser -- src/features/create-chat/create-chat.browser.test.ts`; `pnpm check:boundaries`. Real room/admission outcomes are proven by132, not claimed here.
