...
| Control socket path `~/.codex/app-server-control/app-server-control.sock` | `codex app-server daemon version` with no daemon running returned `Error: failed to connect to /home/everdred/.codex/app-server-control/app-server-control.sock … No such file or directory (os error 2)` | The daemon is not always running. A route that assumes it is will fail; KHA-146 must record start behaviour |
