...
| `ChannelSummary` / `channels.ts` | `roomId`, `title:string|null`, `membership:joining|joined|left|revoked`, `revision` | Empty optional title resolves to null; revision opaque and scoped to one channel projection |
...
| `ChannelPort` | `create({operationId,title})` → ChannelSummary; `prepareIntro({roomId,batchId,messages})` → per-item SendState; `resumeIntro(batchId)` → same states; `send({roomId,clientTxnId,content})`; `timeline({roomId,cursor,limit})` → TimelinePage; `observe(roomId,listener)` → disposer; listener receives full `{room:ChannelSummary,items:TimelineItem[],snapshotRevision,generation}` snapshot (replacement, not delta) | KHA-112 |
