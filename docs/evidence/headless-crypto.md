...
Commands: `pnpm --dir experiments/headless-crypto install --frozen-lockfile`, `build`, `test`, `test:live` (same --dir). Offline: 3 pass. Live: 1 pass, no skips, 46.86 seconds on final strengthened run. Node22.23.2 live run also passed (46.21 seconds): from the experiment directory run `~/.local/share/mise/installs/node/22.23.2/bin/node --import tsx --test tests/interoperability.test.mjs`. Native child processes inherit that same Node22 executable.
