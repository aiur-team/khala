...
| 20 internal server routes | Probed with the agent's binding and without credentials. The pending channel returns 403; traversal and encoded variants, sent unnormalized, return 400/403/404; the hint stream opens (`event: ready`) and stays content-free while new messages arrive. Session exchange with a binding bearer returns 401; channel creation, the owner-only receipts route and the human-only binding Stop route return 403. Stop is mounted as the launcher composes it, and a refused Stop leaves the agent's own read working. The two Make-external journey routes (#383) return 403 to the binding for view and for a well-formed action in either channel. The launcher does not mount them yet, so the suite composes the real journey over the real store to mount them; only the hosted side is the journey's test provider, which a refused request never reaches. Discovery fails on any server route registration it cannot account for. The five app-shell document routes (channel, settings, Make external, channel requests) mount only with a built asset manifest. The tested composition has none, so they answer 404. The static shell they would serve is **not observed**. |
