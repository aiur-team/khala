...
The agent's existing session runs a local owner endpoint. When the human pastes a chat link into that session, the endpoint:
...
| --- | --- | --- |
| OAuth sign-in → chat → existing session bound → endpoint-created agent device joins and sends in the room | live | pass |
| Agent Matrix user is distinct from the human's; the endpoint, not the control plane, holds its device token | live | pass |
| Human-visible steps: only sign-in, "New chat", paste link | live + offline | pass (listed below) |
...
| Another session of the same owner cannot silently take over the room binding (409) | offline | pass |
...
3. Click "New chat".
4. Copy the chat link and paste it into the existing agent session.
...
- Re-binding a room to another session. This returns `binding_conflict`, so an explicit owner re-pair flow is needed.
