...
| disconnect | 6823; watch killed at 6843 | 6844 | 6935 `dequeue` | 8643 | Second message at 8663, after the kill: **not delivered** in 15 s |
...
`claude --version` stdout hashes to `e8c1f798…` and `claude --help` stdout to `ae85d661…` (`evidence/inventory.json`).
...
| Approve the busy-case `Bash` command | human | test scaffolding only; not part of attachment |
...
- **KHA-106 (capability and receipt facts).** Claude 2.1.276 exposes four observable receipt levels: write, native enqueue (transcript only), delivery (`dequeue`, or `queued_command` + `remove` while busy), and consumption. Busy delivery waits for the next tool boundary and does not interrupt tools. There is no delivery acknowledgement to the sender.
