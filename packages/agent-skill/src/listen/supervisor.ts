...
import { decodeBindingId, type BindingId } from '@khala/contracts/delivery/index';

export type ListenerProcessError = 'listener_busy';
...
export class ListenerBusyError extends Error {
  readonly code = 'listener_busy';

  constructor() {
    super('listener_busy');
    this.name = 'ListenerBusyError';
  }
}
...
function validBindingId(value: string): BindingId {
  const decoded = decodeBindingId(value);
  if (!decoded.ok) throw new TypeError('invalid binding id');
