...
| Unknown outcome stays unknown | Recovery lists a release with any dispatch evidence (`dispatching`, `transport_written`, `harness_queued`, `context_consumed` or `outcome_unknown`, correlated or not), or whose dispatcher record is `dispatching`, `accepted` or `outcome_unknown`, as `outcomeUnknownReleases` (never resubmit), apart from `undispatchedReleases`. A dispatcher record that is `completed`, `failed`, `cancelled` or `abandoned` settles the release | `crash.test.ts`, `ledger.test.ts`, `dispatch.test.ts` |
