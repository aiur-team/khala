...
import type {
  CallOptions, DeviceId, Disposer, EventId, MessageContent, ParticipantId, ParticipantView, RoomId, ChannelRejection, ChannelSummary,
...
export type SubstrateEffect<T> =
  ...
  | Readonly<{ kind: 'rejected'; code: ChannelRejection }>
...
export interface ChannelSubstrate {
   ...
   */
  createRoom(input: Readonly<{ operationId: string; title: string | null }>, options?: CallOptions): Promise<SubstrateEffect<ChannelSummary>>;
  findCreatedRoom(input: Readonly<{ operationId: string }>, options?: CallOptions): Promise<CreateLookup>;
  room(roomId: RoomId, options?: CallOptions): Promise<SubstrateRead<ChannelSummary>>;
  /**
...
/** @deprecated Use `ChannelSubstrate`. Kept through the first tagged release containing #163. */
export type RoomSubstrate = ChannelSubstrate;
