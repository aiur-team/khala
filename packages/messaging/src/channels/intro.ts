// Channel intro batches: an ordered, immutable selection of messages with per-item
...
import {
  type CallOptions, type IntroBatch, type OperationResult, type ChannelRejection, type SendState,
  ok, rejected, unavailable,
} from '@khala/contracts/messaging/index';
import { type ChannelContext, authorOf, deviceGate, digestContent, isIdentifier, isRefusal, membershipGate, sameAuthor } from './context';
import { type IntroRecord, type SendItem, journalKey } from './journal';
import { toSendState, transmit } from './send';

type IntroResult = OperationResult<readonly SendState[], ChannelRejection>;

export async function prepareIntro(ctx: ChannelContext, input: IntroBatch, options?: CallOptions): Promise<IntroResult> {
...
export async function resumeIntro(ctx: ChannelContext, batchId: string, options?: CallOptions): Promise<IntroResult> {
...
async function resumeRecord(
  ctx: ChannelContext, batchId: string, record: IntroRecord, revision: string, options: CallOptions | undefined,
): Promise<IntroResult> {
