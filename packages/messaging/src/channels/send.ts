// Single-message channel sends. A client transaction freezes its bytes on first use, so a
// failed or unknown send is retried with the same transaction and content.

import {
  type CallOptions, type MessageContent, type OperationResult, type RoomId, type ChannelRejection, type SendState,
  ok, outcomeUnknown, rejected, unavailable,
} from '@khala/contracts/messaging/index';
import { type ChannelContext, authorOf, deviceGate, digestContent, isIdentifier, isRefusal, membershipGate, safeEffect, sameAuthor } from './context';
...
export async function transmit(
  ctx: ChannelContext, roomId: RoomId, item: SendItem, options: CallOptions | undefined,
): Promise<Readonly<{ item: SendItem; rejection: ChannelRejection | null }>> {
  ...
  let next: SendItem;
  let rejection: ChannelRejection | null = null;
  switch (result.kind) {
    case 'done':
      next = {
        ...item,
...
export async function send(ctx: ChannelContext, input: SendInput, options?: CallOptions): Promise<OperationResult<SendState, ChannelRejection>> {
