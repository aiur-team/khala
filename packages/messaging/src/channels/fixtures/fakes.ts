// Typed fakes for channel tests. They prove module behaviour only, never a
...
import {
  ...
  type OwnerId, type ParticipantId, type ParticipantView, type RoomId, type ChannelRejection, type ChannelSummary,
  decodeContentLimits,
} from '@khala/contracts/messaging/index';
import { createMemoryChannelJournal, createChannelService, type ChannelJournal, type ChannelService } from '../index';
import type { AcceptedEvent, CreateLookup, ChannelSubstrate, SubstrateEffect, SubstratePage, SubstrateRead, SubstrateUpdate } from '../substrate';
...
export type SendMode = 'accept' | 'lose' | 'unavailable' | 'throw' | Readonly<{ rejected: ChannelRejection }>;
export type CreateMode = 'accept' | 'lose' | 'unavailable' | 'throw' | Readonly<{ rejected: ChannelRejection }>;

type Landed = { eventId: EventId; roomId: RoomId; clientTxnId: string; content: MessageContent; deviceId: DeviceId };

export class FakeSubstrate implements ChannelSubstrate {
  readonly rooms = new Map<RoomId, ChannelSummary>();
  readonly byOperation = new Map<string, ChannelSummary>();
  ...
  addRoom(room: Partial<ChannelSummary> = {}): ChannelSummary {
    const summary: ChannelSummary = { roomId: `room-${++this.sequence}` as RoomId, title: null, membership: 'joined', revision: '1', ...room };
    this.rooms.set(summary.roomId, summary);
    return summary;
  }

  async createRoom(input: Readonly<{ operationId: string; title: string | null }>): Promise<SubstrateEffect<ChannelSummary>> {
  ...
  async room(roomId: RoomId): Promise<SubstrateRead<ChannelSummary>> {
...
export function harness(
  options: Partial<{ actor: ParticipantView; substrate: FakeSubstrate; journal: ChannelJournal; principal: AuthPrincipal; newId: () => string }> = {},
): Harness {
  const substrate = options.substrate ?? new FakeSubstrate();
  const device = fakeDevice();
  const journal = options.journal ?? createMemoryChannelJournal();
