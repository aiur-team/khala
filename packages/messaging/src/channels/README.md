# `@khala/messaging/channels`

The `ChannelPort` adapter (KHA-112): channel creation, intro batches, sends and the timeline
projection. Import `createChannelService` from `@khala/messaging/channels/index`. UI callers get
...
| `substrate` | `ChannelSubstrate`, the narrow SDK surface in `substrate.ts`. G-SUBSTRATE is open, so the selected SDK's adapter implements it |
| `journal` | `ChannelJournal`, device-local storage. It holds message bodies, so it is never the shared `ControlStore`. Tabs may share it, so every update is a compare-and-set on the record revision. `createMemoryChannelJournal` is not durable and only suits tests |
...
- **Create.** Only a human actor may create a channel; a delegated agent gets `forbidden`. The
  ...
  retry after the attempt ends, or after its lease expires, looks the underlying Matrix room up with
  ...
  is `operation_mismatch`. An empty title is `null`. The title is Matrix room metadata, and nothing
  here promises that it is encrypted.
- **Intro batches.** `prepareIntro` freezes the channel, author, device, item order, bytes and a
...
The contract's `TimelinePage` and `ChannelSnapshot` carry only decrypted messages. The extra
`observeEntries` view adds explicit `unavailable` placeholders (`missing_key`,
`decryption_failed`, `digest_unavailable`) and `local` entries for own sends the channel has not
...
- Matrix room creation is tagged with the operation ID, and `findCreatedRoom` answers `absent` only
  on proof.
...
History disclosure on a new admission belongs to KHA-113. Channel creation here opens no history
to later members.
