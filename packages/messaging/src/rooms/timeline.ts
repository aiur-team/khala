// Projects SDK events and local sends into the contract's timeline views. Each
...
import {
  ...
  type RoomRejection, type RoomSnapshot, type RoomSummary, type SendState, type TimelineItem, type TimelinePage,
  digestMessageContent, ok, rejected, unavailable,
} from '@khala/contracts/messaging/index';
import { type RoomContext, isIdentifier, safeRead } from './context';
...
/** Full replacement view of one room, including placeholders and local sends. */
export type RoomEntriesView = Readonly<{
  roomId: RoomId;
...
export async function timeline(
  ctx: RoomContext, input: Readonly<{ roomId: RoomId; cursor: string | null; limit: number }>, options?: CallOptions,
): Promise<OperationResult<TimelinePage, RoomRejection>> {
...
/**
 * Live state of one room. Remote events are keyed by event ID; own sends are keyed
 * by transaction ID until the room or the send response names their event.
 */
export class RoomProjection {
  ...
  applyRoom(room: RoomSummary | null): void {
  ...
  entries(generation: number): RoomEntriesView {
  ...
  /** The contract snapshot: known room and decrypted messages only. */
  snapshot(generation: number): RoomSnapshot | null {
