...
/**
 * The descriptor a command runs against when argv names none. Only `mcp-serve`, whose
 * installed entries carry no argv beyond the command, falls back to the composed default.
 */
function defaultDescriptor(command: string, deps: CliDependencies): string | null {
  return command === 'mcp-serve' ? deps.defaultDescriptorPath ?? null : null;
}
