...
export function createMcpResultPostprocessor(
  ...
  return async <Result extends McpToolResult>(input: McpResultPostprocessInput<Result>): Promise<Result> => {
    ...
    try {
      ...
      const batch = hasPreselected
        ? (input as PreselectedPostprocessInput<Result>).preselectedBatch
        : await readConsumerBatch(input as ConsumerPostprocessInput<Result>, softResponseBytes);
...
async function readConsumerBatch<Result extends McpToolResult>(
  ...
  const maxBytes = conservativePayloadBudget(input.responseId, input.primaryResult, softResponseBytes);
...
function conservativePayloadBudget(
  responseId: McpJsonRpcId,
