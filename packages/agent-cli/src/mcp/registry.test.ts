...
describe('MCP tool registry', () => {
  it('registers exactly the existing tools in list order', () => {
    expect(MCP_TOOLS.map(entry => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_list_channels', 'khala_list_agents']);
    expect(toolRegistry.definitions().map(entry => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_list_channels', 'khala_list_agents']);
  });
  ...
  it('adds a tool with one new definition and one registry entry', async () => {
    ...
    expect(list.result.tools.map((entry: { name: string }) => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_list_channels', 'khala_list_agents', 'khala_probe']);
