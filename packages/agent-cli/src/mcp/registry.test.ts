...
describe('MCP tool registry', () => {
  it('registers exactly the existing tools in list order', () => {
    expect(MCP_TOOLS.map(entry => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_listening_mode', 'khala_list_channels', 'khala_list_agents', 'khala_request_channel_access', 'khala_channel_access_status']);
    expect(toolRegistry.definitions().map(entry => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_listening_mode', 'khala_list_channels', 'khala_list_agents', 'khala_request_channel_access', 'khala_channel_access_status']);
  });
  ...
  it('adds a tool with one new definition and one registry entry', async () => {
    ...
    expect(list.result.tools.map((entry: { name: string }) => entry.name)).toEqual(['khala_send', 'khala_read', 'khala_listening_mode', 'khala_list_channels', 'khala_list_agents', 'khala_request_channel_access', 'khala_channel_access_status', 'khala_probe']);
