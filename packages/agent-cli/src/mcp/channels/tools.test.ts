...
describe('MCP channel listing tools', () => {
  it('advertises strict schemas for khala_list_channels and khala_list_agents', async () => {
    ...
    expect(tools.map(tool => tool.name)).toEqual(['khala_send', 'khala_read', 'khala_listening_mode', 'khala_list_channels', 'khala_list_agents', 'khala_request_channel_access', 'khala_channel_access_status', 'khala_create_channel', 'khala_channel_create_status', 'khala_pair']);
    ...
    expect(tools.find(tool => tool.name === 'khala_list_agents')?.inputSchema).toMatchObject({
