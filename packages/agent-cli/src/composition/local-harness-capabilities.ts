...
// unproven. Claude's internal routes are unproven, so every Claude mode stays
// unusable. Any other harness, or any inspection failure, claims nothing.
  ...
  const inspect = (harness: string): Promise<Inspection> => {
    ...
    if (found === undefined) {
      found = harness === 'codex'
        ...
        : Promise.resolve(harness === 'claude'
          ? { capabilities: claudeCapabilities(null, LOCAL_DELIVERY_LIMITS), observation: null }
          : NOTHING);
