...
describe('interactive Codex capabilities', () => {
  ...
  it('claims the native queue notification only while the idle wake works', () => {
    const woken = interactiveCodexCapabilities('0.154.0', limits, { state: 'trusted' }, 'available');
    ...
    const failed = interactiveCodexCapabilities('0.154.0', limits, { state: 'trusted' }, 'unavailable');
