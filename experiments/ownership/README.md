...
This experiment shows how a public channel link can connect to the owner's existing agent session. The human does no technical setup, and a copied link cannot claim the owner's identity. It is feasibility code, not a production auth service. Results and open gates are in `docs/evidence/ownership.md`.
...
| Channel link | Room reference only, no authority | `POST /api/human/chats` by a room member | `GET /c/:link` returns a descriptor with no owner, room ID or secret |
...
| Owner binding | Owner + channel + session + endpoint key | The owner's already signed-in browser at `/api/agent/bind`, redirected to an RFC 8252 loopback URI with PKCE | `OwnershipBoundary.authorize`: session cookie, loopback redirect, admission policy, conflict check |
